...
def main(_argv):
    ...
    #sio.connect('http://192.168.200.39:5000')
	...
	parameters=arucoParams)
        ...
        if(len(objects)>0):
            sio.emit("marker_detected", {"file":image_data.tobytes(), "marker": net.class_names[int(objects[0].label)]})
        else:
            sio.emit("marker_detected", {"file":image_data.tobytes(), "marker": 0})
